<template>
  <div>
    <LoadingDiv
      v-if="isLoading.isLoadingType('lessons')"
      v-for="i in 12"
    ...
    >
      <div
        v-for="i in useLesson.store.lessons"
        @click="$router.push(`/lesson/${i.id}`)"
      ...
      >
        ...
        <div class="aspect-video w-full object-cover bg-black r_8">
          <video
            v-if="i.video"
            class="aspect-video w-full object-cover bg-black r_8"
            controls
          >
        ...
        <div class="p-3 space-y-2">
          ...
          <div class="flex gap-2">
            <button
              class="text-sm font-medium py-1 px-2 bg_cf2 r_8"
              v-for="i in 3"
            >
              3 courses
            </button>
          ...
          <div class="flex items-center text-sm pt-2 pcursor">
            <div
              class="flex items-center -space-x-2 w-[70%] overflow-hidden overflow-x-auto removeScroll"
            >
              <img
                v-for="i in 16"
                class="h-7 w-7 min-w-[28px] r_f object-cover"
